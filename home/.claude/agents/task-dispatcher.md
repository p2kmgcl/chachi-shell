...
description: Reads plan state, picks next task, and writes action.json
permissionMode: dontAsk
tools: Read, Bash
skills: focused-agent, agent-state/ticket, agent-state/plan, agent-state/task, agent-state/action, agent-state/domain-context
model: haiku
---
...
1. Read context files from `.agent-state/` (ticket.json, plan.json, domain-context.json)

2. Check if `.agent-state/task.json` exists.
...
- `BLOCKER:` → write action.json: `{"action": "stop"}`
...
- `VALIDATION_ERROR:` → write action.json: `{"action": "create_incomplete_pr"}`
