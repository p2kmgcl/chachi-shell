...
return function(file_path)
  ...
  -- Helper function to get directory depth (distance from file)
  local function get_distance(config_path, current_dir)
    if config_path == "" then
      return math.huge
    end
    local config_dir = vim.fn.fnamemodify(config_path, ":h")
    local current_parts = vim.split(current_dir, "/")
    local config_parts = vim.split(config_dir, "/")
    
    -- Check if config_dir is a parent of current_dir
    if #config_parts > #current_parts then
      return math.huge
    end
    
    for i = 1, #config_parts do
      if config_parts[i] ~= current_parts[i] then
        return math.huge
      end
    end
    
    return #current_parts - #config_parts
  end
