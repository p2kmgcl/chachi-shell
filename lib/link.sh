...
link_file() {
  ...
  if [ ! -f "$source_path" ]; then
    echo_error "$label" "source '$source_path' is not a file"
    return 1
  fi
  ...
  if [ -d "$target_path" ]; then
    echo_error "$label" "'$target_path' is a directory, it should be a file"
    return 1
  fi
  ...
  if [ -e "$target_path" ]; then
    ...
    if [ -e "$backup_path" ]; then
      echo_error "$label" "'$backup_path' backup file already exists"
      return 3
    fi
