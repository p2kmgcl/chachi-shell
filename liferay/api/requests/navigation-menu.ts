...
export async function createNavigationMenu() {
  ...
  for (const navigationMenu of navigationMenusPage.items || []) {
    if (navigationMenu.id) {
      await api.headlessDelivery.v10.deleteNavigationMenu(navigationMenu.id.toString());
    }
  }
    ...
    navigationMenuItems: [
      {
        ...
        navigationMenuItems: [
          {
            name: 'A', type: 'submenu', navigationMenuItems: [
              {
                name: 'E', type: 'submenu', navigationMenuItems: [
                  {
                    name: 'I', type: 'submenu', navigationMenuItems: [
                      {
                        name: 'O', type: 'submenu', navigationMenuItems: [
                          { name: 'U', type: 'submenu' },
                        ]
                      },
                    ]
