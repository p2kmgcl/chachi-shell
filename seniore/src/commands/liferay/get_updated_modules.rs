...
impl Runnable for Command {
    fn run(&self) -> Result<(), String> {
        ...
        let branch_files = {
            let last_commit =
                tmp::read_file(super::LAST_COMMIT_FILE_NAME).unwrap_or("master".to_string());
        ...
        let module_set = changed_files
            .lines()
            .filter_map(|file| {
                let mut maybe_file_path = Some(super::get_portal_item_path(&format!("/{}", file)));

                while let Some(file_path) = maybe_file_path {
                    if super::is_osgi_module(&file_path) {
